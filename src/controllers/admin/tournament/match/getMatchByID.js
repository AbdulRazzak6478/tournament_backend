...
const getMatchDetails = catchAsync(async (req, res) => {
  try {
    ...
    let matchResponse = await tournamentMatchModel.findOne({
      _id : matchID,
    }).populate([{path : 'venueID' , populate : 'venueClubId'},'matchA','matchB']);
    ...
    let matchDetails = {
        ...
        venue : {
          id : matchResponse?.venueID?._id?.toString(),
          name : matchResponse?.venueID?.venueClubId?.ClubName,
          location : matchResponse?.venueID?.venueClubId?.location_operating,
          city : matchResponse?.venueID?.city,
        },
        referee : matchResponse?.refreeName
    }
    let responsePayload = {
        matchDetails ,
    }
