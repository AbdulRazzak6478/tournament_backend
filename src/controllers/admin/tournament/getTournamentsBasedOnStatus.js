...
const getTournamentsBasedOnStatus = catchAsync(async (req, res) => {
  try {
    ...
    const statusContainer = ["ongoing", "upcoming", "completed","archived"];
    if (!statusContainer.includes(status.toLowerCase())) {
      return res
        ...
        .json(
          failed_response(
            400,
            "status should be anyone from this only ['ongoing','upcoming','completed','archived']",
            {},
            false
          )
        );
    ...
    if (status === "upcoming") {
      tournaments = await tournamentModel.find({ isDeleted: false });
    ...
    if(status === "archived"){
      tournaments = await tournamentModel.find({isDeleted: true});
      if (_.isEmpty(tournaments)) {
        tournaments = [];
      }
    ...
    const tournamentsStatusObject = tournaments?.map((tournament) => {
      ...
      const responseObject = {
        ...
        venue: tournament?.venues.length,
        category: {
        ...
        Remark : tournament?.deleteRemark,
      };
