...
const addParticipantInKnockoutFormatAndReArrangeTournament = async (
  tournament,
  participantName
) => {
  ...
  try {
    ...
    if (isWinnerDeclared) {
      throw new Error(",tournament started cannot add participants");
    }
    ...
    if (tournamentDetails?.gameType === "team") {
      ...
      newParticipant = await tournamentTeamModel.create([teamObj], {
        session: session,
      });
    ...
    if (tournamentDetails?.gameType === "individual") {
      ...
      newParticipant = await tournamentPlayerModel.create([playerObj], {
        session: session,
      });
    ...
    for (let roundData of roundsData) {
      ...
      if (_.isEmpty(matches)) {
        throw new Error(",not able to create matches for the respective round");
      }
    ...
    throw new Error(
      ",Error in adding new participant in knockout format :" + error?.message
    );
  }
};

module.exports = addParticipantInKnockoutFormatAndReArrangeTournament;
