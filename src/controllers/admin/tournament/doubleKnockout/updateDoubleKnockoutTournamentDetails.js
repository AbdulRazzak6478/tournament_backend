...
const updateDoubleKnockoutTournamentDetails = async (data) => {
  ...
  try {
    ...
    if (data.participants !== existingParticipants) {
      ...
      let tournamentFormatDetails = await tournamentDoubleFormat
        ...
        .session(session);
      if (_.isEmpty(tournamentFormatDetails)) {
        throw new Error(", tournament double knockout format not found");
      }
      let roundsBrackets = getBracketsRoundsAndMatches(participantsIds.length);
      // Forming rounds Data;
      ...
      if (tournamentFormatDetails?.gameType === "team") {
        tournamentFormatDetails.totalTeams = participantsIds.length;
        tournamentFormatDetails.teams = participantsIds;
      }
      if (tournamentFormatDetails?.gameType === "individual") {
        tournamentFormatDetails.totalParticipants = participantsIds.length;
        tournamentFormatDetails.participants = participantsIds;
      }
    ...
    console.log(
      ",Error in updating tournament details for double knockout section" +
        error?.message
    );
...
module.exports = updateDoubleKnockoutTournamentDetails;
